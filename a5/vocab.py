...
class VocabEntry(object):
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        sents_padded = pad_sents_char(sents, self['<pad>']) #(batch, max_sent_len, max_word_len)
        sents_var = torch.tensor(sents_padded,dtype=torch.long, device=device).permute(1,0,2).to(device)
        return sents_var
